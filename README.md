...
- **⚡ 统一 Agent Skill**（新增）：将全部 Prompt 模板内嵌进单一 `SKILL.md`，Claude Code / Cursor 自动识别意图并路由执行，**无需手动复制 Prompt，无需区分 LaTeX/Word 版本**。详见 [Part III](#part-iii-统一-agent-skill推荐)。
...
- [为什么要用统一 Skill？](#为什么要用统一-skill)
- [安装方法](#安装方法)
- [支持的任务](#支持的任务无需记忆-prompt)
- [Skill 文件说明](#skill-文件说明)
...
> 🚀 **这是对 Part I + Part II 的整合升级**：将全部 Prompt 模板内嵌进单一 `SKILL.md`，Claude Code / Cursor agent 自动识别意图并路由执行，**无需手动复制 Prompt、无需区分 LaTeX/Word 版本**。

## 为什么要用统一 Skill？

| 原有方式 | 统一 Skill 方式 |
| --- | --- |
| 安装 skill + 手动找 prompt + 复制模板 + 填入文本 | 一句话描述需求，agent 自动执行 |
| LaTeX 和 Word 场景各有一套 prompt，需手动区分 | 自动检测输入格式，无需区分 |
| 多步骤任务（翻译→润色→去AI味）需反复粘贴 | 支持 pipeline chaining，一次完成 |
| 记忆 13 个 prompt 模板的适用场景 | 自然语言描述需求即可触发 |

## 安装方法

```bash
# 克隆你的 fork（或直接在项目目录下操作）
...
或者用 OpenSkills 一键安装（配置完 OpenSkills 后）：
...
直接在 Claude Code / Cursor 的 Agent Chat 中说：

| 你说的话 | Skill 自动执行 |
| --- | --- |
| 「帮我把这段中文翻成英文 LaTeX」 | Template A · CN→EN |
| 「把这段 LaTeX 翻译成中文让我看一下」 | Template B · EN→CN |
| 「这段中文草稿帮我改成论文格式」 | Template C · CN→CN Polish |
| 「这段太长了帮我缩短一点」 | Template D · Shorten |
| 「这段字数不够帮我扩写」 | Template E · Expand |
| 「帮我润色这段英文 LaTeX」 | Template F · Polish EN |
| 「去一下 AI 味」（LaTeX 输入） | Template G · De-AI EN |
| 「去一下 AI 味」（中文输入） | Template H · De-AI CN |
...
└── skills/
    └── ai-research-writing/
        └── SKILL.md    ← 包含全部 13 个 Prompt 模板的路由系统
```

`SKILL.md` 核心设计原则：

- **意图路由**：agent 读取用户意图，自动匹配对应模板执行
- **格式自检**：通过检测 `\begin`、`$$`、`\cite` 等特征自动判断 LaTeX vs 纯文本
- **语言自检**：通过检测中文字符自动判断源语言
- **Pipeline 支持**：多步骤任务自动串联，无需重复粘贴
- **最小修改原则**：所有模板内置"宁缺毋滥"约束，输入已足够好时直接返回并说明
