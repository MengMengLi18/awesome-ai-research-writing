...
> 🚀 **这是对 Part I + Part II 的整合升级**：将全部 Prompt 模板内嵌进单一 `SKILL.md`，agent 自动识别意图并路由执行，无需手动复制 Prompt、无需区分 LaTeX/Word 版本。
...
| LaTeX 和 Word 场景各有一套 prompt | 自动检测输入格式，无需区分 |
...
# 将 SKILL.md 放入目录（SKILL.md 已在本 repo 根目录）
